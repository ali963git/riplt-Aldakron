...
export class AdminAzkarService {
  ...
  async createCategory(adminId: string, dto: CreateAzkarCategoryDto) {
    ...
    await this.auditLog.log(adminId, 'azkar_category.create', 'AzkarCategory', category.id, dto as unknown as Record<string, unknown>);
    return category;
  }
  ...
  async createItem(adminId: string, dto: CreateAzkarItemDto) {
    const category = await this.prisma.azkarCategory.findUnique({ where: { id: dto.categoryId } });
    if (!category) throw new NotFoundException('التصنيف المحدد غير موجود');
    const item = await this.prisma.azkarItem.create({ data: dto });
    await this.auditLog.log(adminId, 'azkar_item.create', 'AzkarItem', item.id, dto as unknown as Record<string, unknown>);
    return item;
  }
  ...
  async updateItem(adminId: string, id: string, dto: UpdateAzkarItemDto) {
    ...
    await this.auditLog.log(adminId, 'azkar_item.update', 'AzkarItem', id, dto as unknown as Record<string, unknown>);
    return updated;
  }
